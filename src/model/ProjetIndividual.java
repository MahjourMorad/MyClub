...
public class ProjetIndividual extends JFrame {
	/**
	 * 
	 */
	...
	private void jButton4ActionPerformed(ActionEvent evt) {// GEN-FIRST:event_jButton4ActionPerformed
		...
		if (jTextField1.getText().equals("")) {
			...
			if (p == 0) {
				String sql = "delete from event where id_pr=? ";
				try {
					pstm = con.prepareStatement(sql);
					pstm.setString(1, jTextField1.getText());
					pstm.execute();
		...
		if (jTextField1.getText().equals("")) {
			...
			if (p == 0) {
				try {
					...
					String sql = "update projets set id='" + value1 + "',titre='" + value2 + "', chef_projet='"
							+ value3 + "',Membres='" + value5 + "',description='"
							+ value6 + "'"
